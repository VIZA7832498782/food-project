...
const DetailMenuFood = () => {
  ...
  useEffect(() => {
    if (food) {
      ...
      const isAlreadyFavorite = favorites.some(item => item.id === food.id && item.type === 'food');
  ...
  const handleFavoriteToggle = () => {
    ...
    if (isFavorite) {
      ...
      const updatedFavorites = favorites.filter(item => !(item.id === food.id && item.type === 'food'));
    ...
    return (
      <div className="w-full min-h-screen flex items-center justify-center bg-gradient-to-b from-orange-50 to-white">
        <div className="text-center">
          ...
          <Link to="/MenuFood" className="text-orange-600 hover:text-orange-700 font-semibold">
            ← Back to Menu
          </Link>
        </div>
      </div>
  ...
  const addToCart = () => {
    ...
    const existingItemIndex = cart.findIndex(item => item.id === food.id && item.type === 'food');
  ...
  return (
    <>
      ...
      {/* Back Button */}
      <Link to="/MenuFood" className="inline-flex items-center gap-2 text-orange-600 hover:text-orange-700 font-semibold mb-8 group">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div>
            <div className="bg-white rounded-3xl shadow-xl p-8 md:p-10">
              ...
              <div className="mb-8">
                <div className="flex items-baseline gap-3">
                  ...
                  <span className="text-gray-500 text-lg line-through">${(food.price * 1.5 * quantity).toFixed(2)}</span>
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-gray-500 ml-4">Available: <span className="font-semibold text-green-600">{food.qty} items</span></span>
                </div>
              </div>
            ...
            <div className="mt-6 bg-gradient-to-r from-orange-100 to-red-100 rounded-2xl p-6">
              ...
              <p className="text-gray-700">Order 3 or more items and get an extra 10% discount + Free dessert!</p>
            </div>
          </div>
        </div>
      </div>
